                ...
                <form action="" method="GET" class="row g-3 align-items-center">
                    <div class="col-md-5">
                        <input type="text" name="search" class="form-control" id=""
                            placeholder="Tìm kiếm tên Thẻ..." value="{{ $search ?? old($search) }}">
                    </div>
                    <div class="col-sm-5">
                        <input type="text" name="mealSearch" class="form-control" placeholder="Tìm theo món ăn... "
                            value="{{ $mealSearch ?? old($mealSearch) }}">
                    </div>
                    <div class="col-md-2">
                            ...
                            <tr onclick="window.location='{{ route('tags.show', $phanTu->id) }}'" style="cursor: pointer;">
                                ...
                                <td class="text-center">
                                    <div class="btn-group" role="group">
                                        <a href="{{ route('tags.show', ['id' => $phanTu->id]) }}"
                                            class="btn btn-sm btn-info rounded  me-3" title="chi tiết"><i
                                                class="bi bi-eye"></i></a>
                                        <a href="{{ route('tags.form', ['id' => $phanTu->id]) }}"
                                            class="btn btn-sm btn-warning rounded  me-3" title="Sửa"><i
                                                class="bi bi-pencil-square"></i></a>
                                        <form action="{{ route('tags.delete', ['id' => $phanTu->id]) }}" method="POST"
                                                        ...
                                                        <div class="col-md-4">
                                                            <div class="form-check">
                                                                ...
                                                                <label class="form-check-label"
                                                                    for="meal_{{ $phanTu->id }}_{{ $meal->id }}">{{ $meal->name }}</label>
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                </div>
                            ...
                            <tr>
                                <td class="text-center text-muted" colspan="6">
                                    @if (!empty($search) && !empty($mealSearch))
                                        ...
                                        và món ăn "<strong>{{ $mealSearch }}</strong>"
                                    @elseif(!empty($search))
                                        Không tìm thấy thẻ nào phù hợp với tên thẻ "<strong>{{ $search }}</strong>"
                                ...
                                <div class="d-flex flex-wrap gap-1">
                                    ...
                                    @else
                                        ...
                                        @foreach ($tagsToShow as $t)
                                            <span class="badge bg-danger text-truncate" title="{{ $t->name }}"
                                        ...
                                        @if ($totalTags > $maxTagsToShow)
                                            <span class="badge bg-danger"
                                                title="Còn {{ $totalTags - $maxTagsToShow }} thẻ khác">
                                                ...
                                            </span>
                                        @endif
                                    @endif
