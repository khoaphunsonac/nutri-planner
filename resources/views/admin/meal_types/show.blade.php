...
<div class="container py-4">
  ...
  <div class="d-flex justify-content-between align-items-start mb-3">
    ...
    <div class="d-flex gap-2">
      <a href="{{ route('admin.meal_types.index') }}" class="btn btn-light" title="Danh sách" aria-label="Danh sách">
        <i class="bi bi-arrow-left"></i>
      </a>
      <a href="{{ route('admin.meal_types.edit', $item->id) }}" class="btn btn-outline-primary" title="Sửa" aria-label="Sửa">
  ...
  @php
    ...
    $relatedCount = $relatedCount
      ?? ($item->dishes_count ?? null)
      ?? (isset($relatedDishes)
            ? (is_countable($relatedDishes) ? count($relatedDishes) : (method_exists($relatedDishes,'count') ? $relatedDishes->count() : 0))
            : ($usedCount ?? null))
      ?? 0;
  @endphp
    ...
    <div class="col-6 col-md-3">
      <div class="card shadow-sm rounded-4 h-100">
        <div class="card-body py-3">
          ...
          <div class="fw-semibold text-success text-truncate" title="{{ $item->name }}">{{ $item->name }}</div>
        </div>
      </div>
    </div>
    <div class="col-6 col-md-3">
  ...
  <div class="card border-0 shadow-sm rounded-4 mb-4">
    <div class="card-body p-4">
      ...
      <div class="row gy-3">
        <div class="col-md-6">
          <div class="d-flex">
            <div class="text-muted fw-medium" style="min-width:140px">Slug</div>
            <div class="flex-grow-1 text-truncate" title="{{ $item->slug ?? '' }}">{{ $item->slug ?? '—' }}</div>
          </div>
        </div>
        <div class="col-md-6">
          <div class="d-flex">
        ...
        <div class="col-md-6">
          <div class="d-flex">
            <div class="text-muted fw-medium" style="min-width:140px">Mô tả</div>
  ...
  <div class="card border-0 shadow-sm rounded-4">
    <div class="card-body p-4">
      <h6 class="text-uppercase text-muted fw-semibold mb-3"><i class="bi bi-basket me-1"></i>Món được phân loại</h6>

      @if(isset($relatedDishes) && (is_countable($relatedDishes) ? count($relatedDishes) : (method_exists($relatedDishes,'count') ? $relatedDishes->count() : 0)))
          ...
          <table class="table align-middle mb-0">
            ...
            <tbody>
              @foreach($relatedDishes as $d)
                <tr>
                  <td>{{ $d->id }}</td>
                  <td class="text-truncate" style="max-width:360px" title="{{ $d->name }}">{{ $d->name }}</td>
                  <td class="text-muted">{{ $d->calories ?? '—' }}</td>
                  <td class="text-muted">{{ $d->updated_at?->format('d/m/Y H:i') ?? '—' }}</td>
                  <td class="text-end">
                    <div class="btn-group btn-group-sm">
                      <a href="{{ route('admin.dishes.show',$d->id) }}" class="btn btn-outline-secondary" title="Xem"><i class="bi bi-eye"></i></a>
                      <a href="{{ route('admin.dishes.edit',$d->id) }}" class="btn btn-outline-primary" title="Sửa"><i class="bi bi-pencil"></i></a>
                    </div>
                  </td>
                </tr>
              @endforeach
